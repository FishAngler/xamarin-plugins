    ...
    public class PushNotificationGcmListener : GcmListenerService
    {
        ...
        public override void OnMessageReceived(string from,Bundle extras)
        {
            if (extras != null && !extras.IsEmpty)
            {
                    ...
                    var values = ProcessBundle(extras, parameters);
        ...
        JObject ProcessBundle(Bundle extras, Dictionary<string, object> parameters)
        {
            ...
            foreach (var key in extras.KeySet())
            {
                ...
                if (value is Bundle)
                {
                    ProcessBundle(value as Bundle, parameters);
                }
                else
                {
                    string stringValue = value.ToString();
                    if (CrossPushNotification.ValidateJSON(stringValue))
                    {
                        values.Add(key, JObject.Parse(stringValue));
                    }
                    else
                    {
